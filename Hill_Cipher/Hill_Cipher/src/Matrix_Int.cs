   ...
   public class Matrix_Int
    {
        ...
        public Matrix_Int(int n)
        {
            ...
            matrix_inverse = new int[size_matrix, size_matrix];
        }
        ...
        public void CreateInverseMatrix()
        {
            ...
            int a = this.Determinant();
            if (a == 0) { MessageBox.Show("Ma trận không khả nghịch"); }
        ...
        {
            if (this.size_matrix == 1)
            {
                return this.matrix[0, 0];
            }
            if (this.size_matrix == 2)
            {
        ...
        {
            Matrix_Int new_matrix = new Matrix_Int(size_matrix);
            for (int i = 0; i < size_matrix; i++)
            {
        ...
        public Matrix_Int Adjugate()
        {
            ...
            Matrix_Int retursize_matrix = new Matrix_Int(size_matrix);
            int flag_i = 1;
            for (int i = 0; i < size_matrix; i++)
            {
                int flag_j = 1;
                for (int j = 0; j < size_matrix; j++)
                {
                    ...
                    //x in [0,26]
                    retursize_matrix.matrix[j, i] = x % 26;
                    flag_j *= -1;
                }
                flag_i *= -1;
            }
            return retursize_matrix;
        }
